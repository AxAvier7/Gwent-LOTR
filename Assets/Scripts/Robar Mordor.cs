...
public void OnClick()
{
    for(int i = 0; i < 10; i++)
{
GameObject Card = Instantiate(Mordor[Random.Range(0, Mordor.Count)], new Vector2(0,0), Quaternion.identity);
Card.transform.SetParent(Mano.transform, false);
//aca se instancian las cartas que se metieron en una lista y se las ubica en la zona de la mano rival
}
}
