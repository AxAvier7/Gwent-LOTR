public class LexicalAnalyzer
{
    ...
    private bool MatchText(TokenReader stream, List<Token> tokens, List<ErrorRekon> errors)
    {
        foreach (var start in Texts.Keys.OrderByDescending(k => k.Length))
        {
            ...
            if (stream.Match(start))
            {
                if (!stream.ReadUntil(Texts[start], out text))
                    errors.Add(new ErrorRekon(stream.Location, ErrorRekon.ErrorID.Expected, Texts[start]));
    ...
    public IEnumerable<Token> GetTokens(string fileName, string code, List<ErrorRekon> errors)
    {
        ...
        while (!stream.EOF)
        {
            ...
            if (stream.ReadNumber(out value))
            {
                ...
                if (!double.TryParse(value, out d))
                    errors.Add(new ErrorRekon(stream.Location, ErrorRekon.ErrorID.Invalid, "Number format"));
            ...
            errors.Add(new ErrorRekon(stream.Location, ErrorRekon.ErrorID.Unknown, unkOp.ToString()));
        }
